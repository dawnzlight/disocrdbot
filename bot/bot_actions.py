...
def pull_requests_summary_action(github, gemini):
    """
    プルリクエストの要約を行う関数
    :param
        github: Github
    ...
    for i, pull_request in enumerate(pull_requests):
        ...
        if len(pull_request['reviewers']) == 0:
...
def pull_request_summary_action(github, gemini, pull_request_number):

    pull_request = github.get_pull_request('all-you-can-drink', pull_request_number)
    ...
    prompt = 'プルリクエストの変更ファイルを見て、その内容について要約してください。' + \
        f"5行程度にまとめてください。また回答は「{pull_request['user']}さんからのプルリクだよ。」で始めてください。" \
        '変更があったファイルは以下の通りです。元気よくお願いします！！' + \
        ' '.join([f"{file['file_name']}、追加された行：{file['diff']['plus']}、削除された行：{file['diff']['minus']}" for file in changed_files])
